<script lang=ts>
    ...
    async function createSetting(){
        try {
            ...
            console.debug("created Settings")
        } catch (err) {
    ...
    onMount(()=>{
        if(!loggedIn){goto("/")}
        getSettings().then(()=>{
                console.debug("settings.length:"+settings.length)
    ...
    function toggleOption(option: trashTypes) {
        if (tempSettings.Relevante_Typen.includes(option)) {
            tempSettings.Relevante_Typen = tempSettings.Relevante_Typen.filter(o => o !== option);
        } else {
            tempSettings.Relevante_Typen = [...tempSettings.Relevante_Typen, option];
        }
    }
...
<div class="max-w-xl mx-auto bg-white rounded-2xl shadow p-6 space-y-4">
    ...
    <!-- Ausgewählte Mülltypen -->
        <div class="flex flex-wrap gap-2">
            {#each tempSettings.Relevante_Typen as option}
            ...
            {#each trashOptions as option}
                <button
                    ...
                    class="px-3 py-1 rounded-full border text-sm 
                        transition 
                        {tempSettings.Relevante_Typen.includes(option) 
    ...
    <select bind:value={tempSettings.Benachrichtigungsart} class="select">
        <!--TODO -->
        <option value="Email">Email</option>
    </select>
    <!--Zeitvorlauf-->
    <h2 class="text-lg font-semibold text-gray-800">Um wie viel Uhr am Vortag möchtest du erinnert werden?</h2>
    <input bind:value={tempSettings.Erinnerungs_Vorlauf} type="time" class="input" placeholder="1200"/>
    ...
    <div>
        ...
        <input bind:value={tempSettings.Strasse} type="text" placeholder={tempSettings.Strasse} class="input" />
        <h3 class="text-base font-semibold text-gray-800">Hausnummer</h3>
        <input bind:value={tempSettings.Hausnummer} type="text" placeholder={tempSettings.Hausnummer.toString()} class="input" />
        <div></div>
    </div>
